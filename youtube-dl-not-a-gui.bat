...
"%directoryPath%youtube-dl\youtube-dl.exe" -U | MORE
FOR %%A IN (%videoURL%) DO (
	"%directoryPath%youtube-dl\youtube-dl.exe" --no-check-certificate --no-warnings -f bestvideo+bestaudio/best -o "%userprofile%\Videos\%%(uploader)s - %%(title)s.%%(ext)s" --ffmpeg-location "%directoryPath%ffmpeg\ffmpeg.exe" "%%A"
)
